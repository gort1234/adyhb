...
const Camera = (() => {
  const state = {
    x: 0, y: 0, zoom: 1,           // current (rendered) values
    targetX: 0, targetY: 0, targetZoom: 1, // where we're smoothing toward
  };

  const MIN_ZOOM = 0.2;
  const MAX_ZOOM = 3;
  const SMOOTH = Utils.prefersReducedMotion ? 1 : 0.22;
  ...
  function worldToScreen(wx, wy) {
    return {
      x: (wx - state.x) * state.zoom + viewportW / 2,
      y: (wy - state.y) * state.zoom + viewportH / 2,
    };
  }

  function screenToWorld(sx, sy) {
    return {
      x: (sx - viewportW / 2) / state.zoom + state.x,
      y: (sy - viewportH / 2) / state.zoom + state.y,
    };
  }
  ...
  function zoomAt(sx, sy, factor) {
    ...
    // Preview the new zoom against current x/y to compute the correction,
    // the smoothing loop will ease the rest of the way in.
    const testZoom = state.targetZoom;
    const afterX = (sx - viewportW / 2) / testZoom + state.targetX;
    const afterY = (sy - viewportH / 2) / testZoom + state.targetY;
  ...
  // Advance current values toward targets. Returns true if anything moved.
  function tick() {
    ...
    if (moving) {
      state.x = Utils.lerp(state.x, state.targetX, SMOOTH);
      state.y = Utils.lerp(state.y, state.targetY, SMOOTH);
      state.zoom = Utils.lerp(state.zoom, state.targetZoom, SMOOTH);
...
const Renderer = (() => {
  ...
  const DOT_COLOR = 'rgba(234, 230, 219, 0.09)';
  const DOT_COLOR_STRONG = 'rgba(234, 230, 219, 0.16)';
...
const App = (() => {
  ...
  let appEl, canvasEl, glowEl, worldLayerEl;
  ...
  function loop() {
    ...
    // Matches Camera.worldToScreen: screen = (world - cam) * zoom + center
